...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        print(f"Fee estimates, super-stingy miner: {[str(e) for e in all_estimates]}")
        ...
        all_estimates = [ self.nodes[0].estimatefee(i) for i in range(1,20) ]
        print(f"Fee estimates, more generous miner: {[str(e) for e in all_estimates]}")
        for e in filter(lambda x: x >= 0, all_estimates):
            if float(e)+delta < min(fees_per_kb) or float(e)-delta > max(fees_per_kb):
                raise AssertionError("Estimated fee (%f) out of range (%f,%f)"%(float(e), min_fee_kb, max_fee_kb))
        ...
        print(f"Final fee estimates: {[str(e) for e in final_estimates]}")
