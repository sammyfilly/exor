...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        response = http_get_call(
            url.hostname,
            url.port,
            f'/rest/block/{bb_hash}{self.FORMAT_SEPARATOR}bin',
            True,
        )
        ...
        json_string = http_get_call(
            url.hostname,
            url.port,
            f'/rest/block/{bb_hash}{self.FORMAT_SEPARATOR}json',
        )
        ...
        json_string = http_get_call(
            url.hostname,
            url.port,
            f'/rest/tx/{tx_hash}{self.FORMAT_SEPARATOR}json',
        )
        json_obj = json.loads(json_string)
        ...
        hex_string = http_get_call(
            url.hostname,
            url.port,
            f'/rest/tx/{tx_hash}{self.FORMAT_SEPARATOR}hex',
            True,
        )
        assert_equal(response.status, 200)
        assert_greater_than(int(response.getheader('content-length')), 10)
        ...
        json_string = http_get_call(
            url.hostname,
            url.port,
            f'/rest/block/{newblockhash[0]}{self.FORMAT_SEPARATOR}json',
        )
        json_obj = json.loads(json_string)
        ...
        json_string = http_get_call(
            url.hostname,
            url.port,
            f'/rest/block/notxdetails/{newblockhash[0]}{self.FORMAT_SEPARATOR}json',
        )
        json_obj = json.loads(json_string)
        for tx in txs:
